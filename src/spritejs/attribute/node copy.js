  ...
  [...transformMap].forEach(([key, value]) => {
    ...
    if (key === "matrix") {
      ...
      m = mat2d.multiply(
        m,
        m,
        mat2d.fromValues(1, Math.tan(y), Math.tan(x), 1, 0, 0)
      );
    } else {
      mat2d[key](m, m, value);
    }
...
export default class Node {
  ...
  constructor(subject) {
    ...
    this[_transformMatrix] = mat2d.fromValues(1, 0, 0, 1, 0, 0);
    ...
    Object.defineProperty(subject, "transformMatrix", {
      get: () => {
        if (!this[_transformMatrix]) {
          this[_transformMatrix] = getMatrix(
            this[_transforms],
  ...
  set transform(value) {
    ...
    if (this[setAttribute]("transform", value, false)) {
      ...
      if (_matrixValue) {
        ...
        const transforms = value.match(
          /(matrix|translate|rotate|scale|skew)\([^()]+\)/g
        );
        if (transforms) {
          ...
          for (let i = 0; i < transforms.length; i++) {
            ...
            if (matched) {
              ...
              if (method === "rotate")
                value = (Math.PI * parseFloat(value)) / 180;
              else
                value = value
                  .trim()
                  .split(/[\s,]+/)
