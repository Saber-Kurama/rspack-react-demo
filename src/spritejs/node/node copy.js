...
export default class Node {
  ...
  get renderMatrix() {
    ...
    if (parent) {
      ...
      if (renderMatrix) {
        ...
        m = mat2d.multiply(Array.of(0, 0, 0, 0, 0, 0), renderMatrix, m);
      }
    }
    return m;
  }
  ...
  dispatchEvent(event) {
    ...
    for (let i = elements.length - 1; i >= 0; i--) {
      ...
      const listeners =
        element[_captureEventListeners] &&
        element[_captureEventListeners][type];
    ...
    if (!event.cancelBubble) {
      for (let i = 0; i < elements.length; i++) {
        const element = elements[i];
        const listeners =
          element[_eventListeners] && element[_eventListeners][type];
        if (listeners && listeners.length) {
          event.currentTarget = element;
          listeners.forEach(({ listener, once }) => {
            listener.call(this, event);
